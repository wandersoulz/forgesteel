...
export class ArtifactData {
	static bladeOfAThousandYears: ItemInterface = ElementFactory.createItem({
...
Though its size and make are often debated, the sword is consistently described as having a crossguard made from pure starlight. When wielded, a brilliant metal blade springs forth from that hilt, suiting the holder's taste in weapons. Those who touch the blade are filled with the vigor and power of the heroes who have held it before.`,
	...
	static encepter: ItemInterface = ElementFactory.createItem({
		id: 'item-encepter',
		name: 'Encepter',
...
The Encepter is said to have first manifested in a young world doomed to apocalypse--unless every last inhabitant of that world could stand together. The scepter is said to impose either dominion or obliteration over any threat its light is drawn around. Today, it is most commonly known as a bad omen, and should the Encepter reveal itself, folk know that the world teeters on the brink of destruction. Whether any of the stories are true, few can say, for the only living eyes that have witnessed the Encepter belong to dragons deep in slumber.`,
		featuresByLevel: [
			{
				...
				features: [
					ElementFactory.FeatureFactory.create({
						...
						description:
							'The one who wields the Encepter is always cast in a brilliant glow. Any power roll made by the wielder that uses Presence automatically achieves a tier 3 outcome, though the wielder can still roll in an attempt to score a critical success or critical hit.',
					}),
					ElementFactory.FeatureFactory.create({
						id: 'item-encepter-2',
						name: "Champion's Lasso",
						description:
							"As a free maneuver, the wielder of the scepter can trigger a glowing line of light that traces their path as they move, or can dismiss the glowing line. If the wielder crosses over this line, each creature and object of the wielder's choice enclosed inside the line and within 2 squares above and below it are considered lassoed by the Encepter. Creatures remain lassoed until the lasso is released or until a new line is drawn.",
					}),
					ElementFactory.FeatureFactory.create({
						id: 'item-encepter-3',
						name: 'Dominion',
						description:
							"Each creature lassoed by the Encepter is restrained and can't teleport. A creature caught in midair while lassoed stays in place rather than falling.",
					}),
					ElementFactory.FeatureFactory.create({
						id: 'item-encepter-4',
						name: 'Obliteration',
						description:
							'As a main action, the wielder raises the Encepter to the sky. Each target lassoed by the Encepter erupts in a prismatic burst of light, taking 10 psychic damage for each square horizontally encircled by the lasso. The lasso is then immediately released.',
					}),
					ElementFactory.FeatureFactory.create({
						id: 'item-encepter-5',
						name: "At World's End",
						description:
							'If the Encepter was not taken from its cyclonic resting place with the purpose of vanquishing a terrible peril, then a terrible peril emerges to threaten the world within 3 days of the scepter being taken.',
					}),
				],
			},
		],
	...
	static mortalCoil: ItemInterface = ElementFactory.createItem({
		id: 'item-mortal-coil',
		name: 'Mortal Coil',
		description:
		...
		featuresByLevel: [
			{
				...
				features: [
					...
					ElementFactory.FeatureFactory.create({
						...
						name: "Get Busy Dyin'",
						description:
							'Each creature in the penumbra ages at 10 times the usual rate, and diseases and poisons affecting creatures in the penumbra run their course at 10 times their usual rate. A creature with no natural maximum lifespan permanently reduces their Stamina maximum by 10 each year. Each creature can undertake one additional respite activity during each respite.',
					}),
					ElementFactory.FeatureFactory.create({
						id: 'item-mortal-coil-3',
						name: 'If You Meet God on the Road, Kill Them',
						description:
							"Every non-mortal entity in the penumbra is granted the gift of mortality. Previously immortal or invulnerable entities--from planar creatures to the gods themselves--can be killed while in the penumbra, though not necessarily easily. Any creature or entity who dies in the Mortal Coil's penumbra experiences perfect death. They are permanently, irrevocably dead, and no magic, psionics, or technology can restore them to life.",
					}),
					ElementFactory.FeatureFactory.create({
						id: 'item-mortal-coil-4',
						name: 'Perpetual Motion',
						description:
							"If there are ever no creatures or entities within the Mortal Coil's penumbra, the radius of the penumbra doubles.",
					}),
					ElementFactory.FeatureFactory.create({
						id: 'item-mortal-coil-5',
						name: 'Beneath Contempt',
						description:
							'Deities and their servants always overlook the Mortal Coil and its host--either unable to notice it, or not considering it a threat. If the artifact is somehow destroyed or unmade through godly power, it consumes a year of life from every humanoid in the manifold where it was destroyed, then reforms in a hidden place.',
					}),
				],
			},
		],

...
export class IntriguePerkData {
	static criminalContacts: PerkInterface = {
	...
	static forgettableFace: PerkInterface = {
	...
	static gumUpTheWorks: PerkInterface = {
		...
		data: {
			ability: ElementFactory.createAbility({
				...
				type: ElementFactory.AbilityTypeFactory.createTrigger(
					'A mundane trap activates within 3 squares of you.'
				),
				distance: [ElementFactory.DistanceFactory.createSelf()],
				target: 'Self',
				sections: [
					ElementFactory.createAbilitySectionText(
						'You can move up to 3 squares toward the trap. If this movement brings you adjacent to any of the trap’s mechanisms, you can jam the trap, preventing it from activating. As long as you stay adjacent to the mechanism, the trap can’t go off unless an attempt to disarm it fails.'
					),
				],
			}),
		},
	...
	static masterOfDisguise: PerkInterface = {
	...
	static slippedLead: PerkInterface = {
	...
	static trainedThief: PerkInterface = {

...
export const punisher: SubClassInterface = {
	id: 'beastheart-sub-3',
	name: 'Punisher',
	...
	featuresByLevel: [
		{
			...
			features: [
				...
				ElementFactory.FeatureFactory.createPackageContent({
					...
					description:
						'Your companion slides the target up to a number of squares equal to their Might score.',
					tag: 'feral-strike',
				}),
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						type: ElementFactory.AbilityTypeFactory.createTrigger(
							'An enemy adjacent to you deals damage to a creature.'
						),
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'You deal damage equal to your Might score to the target and push them up to a number of squares equal to your Might score + 1. If this movement causes the enemy to move farther from the creature they damaged, the triggering damage is halved.'
							),
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						type: ElementFactory.AbilityTypeFactory.createTrigger(
							'A creature being force moved by another creature enters a space adjacent to you.',
							{ free: true }
						),
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'You end the forced movement. You can then push the creature up to a number of squares equal to your Might score + 1. The creature takes 1 damage for each square they are moved in this way.'
							),
							ElementFactory.createAbilitySectionSpend({
							...
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										...
										ElementFactory.createAbilitySectionText(
											'If the target is force moved at least 1 square, at the end of this movement an enemy adjacent to the target is also targeted by this ability’s power roll but not this additional effect.'
										),
									],
								}),
							}),
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'Your companion and an ally within range can gain two surges, spend up to two Recoveries, and end one (EoT) or (Save Ends) condition or effect on themselves.'
										),
									],
								}),
							}),
		...
		},
		{
			...
			features: [
				ElementFactory.FeatureFactory.create({
					...
					name: 'Self Sacrifice',
					description:
						'When you or your companion uses Swat Away and halves an attack’s damage, they can take the remaining damage instead of the original target. The damage is transferred before immunity and weakness is applied.',
				}),
			],
		},
		{
					...
					options: [
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									description:
										'Roaring like a pack of wild beasts, your companion and your allies rush toward the foe.',
									type: ElementFactory.AbilityTypeFactory.createManeuver(),
									keywords: [AbilityKeyword.Companion],
									distance: [ElementFactory.DistanceFactory.createSelf()],
									target: 'Self',
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'Your companion shifts up to their speed and can make a free strike. If within 10 squares of the square from which this movement started, you and up to 10 allies can also shift up to their speed and make free strikes.'
										),
									],
								}),
							}),
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'Your companion shifts up to their speed. Your companion makes one power roll that targets each enemy your companion comes adjacent to during the shift. If your companion only targets one enemy with this ability, the power roll has an edge.'
										),
										ElementFactory.createAbilitySectionRoll(
											ElementFactory.createPowerRoll({
												characteristic: Characteristic.Might,
		...
		},
		{
			...
			features: [
				ElementFactory.FeatureFactory.create({
					...
					name: 'Overhand Throw',
					description:
						'When you or your companion uses a maneuver that deals damage, the damage increases by 2. When you or your companion pushes a creature, the push is a vertical push.',
				}),
			],
		},
		{

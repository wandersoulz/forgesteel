...
export const prowler: SubClassInterface = {
	id: 'beastheart-sub-2',
	name: 'Prowler',
	description:
		'You are an unseen ambusher that strikes from the shadows. Your prey is dead before they even know you’re there.',
	featuresByLevel: [
		{
			level: 1,
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						description:
							'While everyone’s eyes are drawn to your foe, you take the opportunity to blend into the scenery.',
						type: ElementFactory.AbilityTypeFactory.createTrigger(
							'An enemy deals damage to a creature other than you.'
						),
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'You become invisible, use the Hide maneuver, and move up to a number of squares equal to your Intuition score, in any order. You remain invisible until the end of your next turn or you deal damage.'
							),
		...
		},
		{
			...
			features: [
				...
				ElementFactory.FeatureFactory.create({
					...
					name: 'Keen Smell',
					description:
						'While a creature is adjacent to your companion, the creature can’t be concealed or hidden from your companion.',
				}),
							...
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										...
										ElementFactory.createAbilitySectionText(
											'Your companion enters the target’s space. Until your companion is no longer in the target’s space, your companion can end their turn in the target’s space, strikes against your companion also affect the target, and your strikes against the target gain an edge.'
										),
									],
								}),
							}),
		...
		},
		{
			...
			features: [
				ElementFactory.FeatureFactory.create({
					...
					description:
						'You or your companion can use While No One’s Looking even when targeted by the triggering attack.',
				}),
			],
		},
		{
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									name: 'Raking Lunge',
									description:
										'Your companion ducks under your enemy’s guard and rakes open their soft vitals, leaving them vulnerable to further attacks.',
									type: ElementFactory.AbilityTypeFactory.createMain(),
									keywords: [
										AbilityKeyword.Companion,
										AbilityKeyword.Melee,
									...
									sections: [
										...
										ElementFactory.createAbilitySectionText(
											'While the target is bleeding, they have damage weakness 5.'
										),
									],
								}),
							}),
					...
					options: [
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									name: 'Chaos Duel',
									description:
										'You or your companion drag your chosen foe into storms of the Primordial Plane.',
									type: ElementFactory.AbilityTypeFactory.createMain(),

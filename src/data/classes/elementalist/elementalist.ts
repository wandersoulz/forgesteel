...
export const elementalist: HeroClassInterface = {
...
As an elementalist, you can unleash your wrath across a field of foes, put an enemy exactly where you want them, debilitate foes with harmful effects, ward yourself and allies against danger, manipulate terrain, warp space, and more. Your choice of elemental specialization determines which of these things you do best.`,
	...
	subclassCount: 1,
	primaryCharacteristicsOptions: [[Characteristic.Reason]],
				...
				ElementFactory.FeatureFactory.createHeroicResource({
					...
					gains: [
						...
						},
						{
							tag: 'take-damage',
							trigger:
								'The first time in a round that you or a creature within 10 of you takes damage that isn’t untyped or holy',
							value: '1',
						},
					],
				}),
					...
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText(
								'When you make this strike, choose the damage type from one of the following options: acid, cold, corruption, fire, lightning, poison, or sonic.'
							),
						],
					}),
				}),
					...
					options: [
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createMultiple({
								...
								description:
									'You gain a bonus to speed and to the distance you can shift when you take the Disengage move action.',
								features: [
									ElementFactory.FeatureFactory.createBonus({
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									description:
										'You store green energy within your body that allows you to produce powerful vines when you’re in danger.',
									type: ElementFactory.AbilityTypeFactory.createTrigger(
										'A creature within a number of squares equal to your Reason score deals damage to you,',
										{ free: true }
									),
									distance: [ElementFactory.DistanceFactory.createSelf()],
									target: 'Self',
									sections: [
										ElementFactory.createAbilitySectionText(
											'You slide the attacking creature up to a number of squares equal to your Reason score.'
										),
									],
								}),
							}),
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'You push that creature a number of squares equal to twice your Reason score.'
										),
									],
								}),
							}),
	...
	abilities: [
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'The ground beneath the area becomes difficult terrain for enemies.'
				),
			],
		}),
		ElementFactory.createAbility({
...
* Each enemy who enters the area for the first time in a combat round or starts their turn there takes damage equal to your Reason score.`),
		...
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'The target uses their signature ability against a creature of your choice. This signature ability can target the creature even if it usually wouldn’t. You then make a power roll against the target of this ability.'
				),
				ElementFactory.createAbilitySectionRoll(
					ElementFactory.createPowerRoll({
						characteristic: [Characteristic.Reason],
		...
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'The target is teleported to another space within distance. Make a power roll that affects each enemy adjacent to the target’s new space.'
				),
				ElementFactory.createAbilitySectionRoll(
					ElementFactory.createPowerRoll({
						characteristic: [Characteristic.Reason],
		...
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'The number of creatures you target with this ability is determined by your power roll.'
				),
				ElementFactory.createAbilitySectionRoll(
					ElementFactory.createPowerRoll({
						characteristic: [Characteristic.Reason],
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText('The ground in or directly beneath the area drops 3 squares.'),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'Until the end of your next turn, each ally in the area has each of their characteristic scores treated as 1 higher for the purpose of resisting potencies, and has a +1 bonus to saving throws.'
				),
				ElementFactory.createAbilitySectionSpend({
					name: 'Persist',
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'When the target ends their next turn, or if they drop to 0 Stamina before then, each enemy adjacent to them takes fire damage equal to twice your Reason score. Each affected enemy then gains this same effect.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			distance: [ElementFactory.DistanceFactory.create({ type: AbilityDistanceType.Cube, value: 4, within: 10 })],
			target: 'Each enemy in the area',
			cost: 9,
			minLevel: 5,
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'The area lasts until the start of your next turn. It is difficult terrain for enemies, and you and your allies have concealment while in the area.'
				),
				ElementFactory.createAbilitySectionSpend({
					name: 'Persist',
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'The area is difficult terrain until the start of your next turn. Each enemy who ends their turn in the area is restrained (save ends).'
				),
				ElementFactory.createAbilitySectionSpend({
					name: 'Persist',
		...
		ElementFactory.createAbility({
			...
			minLevel: 6,
			sections: [
				ElementFactory.createAbilitySectionText(`Until the start of your next turn, the target has the following benefits:
...
* They can use their highest characteristic instead of Might for Might power rolls.`),
		...
		ElementFactory.createAbility({
			...
			distance: [ElementFactory.DistanceFactory.create({ type: AbilityDistanceType.Burst, value: 4 })],
			target: 'Special',
			cost: 9,
			minLevel: 6,
			sections: [
...
Each enemy who ends their turn within 3 squares of the tree is restrained until the end of their next turn. A creature restrained this way can use a main action to end the effect early.`),
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'You create a fissure in the ground adjacent to you that is a 10 × 2 line and 6 squares deep. Each creature in the area who is prone and size 2 or smaller falls in. Other creatures can enter the fissure or can shift to the nearest unoccupied space of their choice outside it.'
				),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'You use up to three heroic abilities whose essence costs total 11 or less, spending no additional essence beyond the cost of this ability. You can shift up to 2 squares between your use of each ability.'
				),
			],
		}),
		ElementFactory.createAbility({
	...
	subclasses: [earth, fire, green, voidSubclass],
	level: 1,
	characteristics: [],
};

...
export const conduit: HeroClassInterface = {
	id: 'class-conduit',
	name: 'Conduit',
	description: `
...
As a conduit, you heal and buff your allies, and debuff your foes while smiting them with divine magic. The spark of divinity within you shines, filling your enemies with awe and making you more worldly and aware.`,
	...
	subclassName: '',
	subclassCount: 0,
	primaryCharacteristicsOptions: [[Characteristic.Intuition]],
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText('You can have this ability deal holy damage.'),
						],
					}),
				}),
					...
					options: [
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									target: 'One ally',
									sections: [
										ElementFactory.createAbilitySectionText('The power roll gains an edge.'),
										ElementFactory.createAbilitySectionSpend({
	...
	abilities: [
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText('You or one ally within distance can spend a Recovery.'),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'Each target is frightened of you or a creature you choose within distance.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			target: 'One ally',
			cost: 7,
			sections: [ElementFactory.createAbilitySectionText('The target gains 20 temporary Stamina and 3 surges.')],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText('One ally within distance can spend any number of Recoveries.'),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText('Each ally in the area can spend a Recovery.'),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter, whenever you or any ally damages the target using an ability, that creature can spend a Recovery. If the target is reduced to 0 Stamina before the end of the encounter, you can use a free triggered action to move this effect to another creature within distance.'
				),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'The target is removed from the encounter map until the start of their next turn and can spend any number of Recoveries. At the start of their turn, the target reappears in the space they left or the nearest unoccupied space of their choice.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'The first time the target is dying or winded before the end of the encounter, each enemy within 5 squares of them takes 15 holy damage.'
				),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			type: ElementFactory.AbilityTypeFactory.createManeuver(),
			keywords: [AbilityKeyword.Area, AbilityKeyword.Magic],
			distance: [ElementFactory.DistanceFactory.create({ type: AbilityDistanceType.Aura, value: 5 })],
			target: 'Self and each ally in the area',
			cost: 11,
			sections: [
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter, any ability roll made against a target takes a bane and each target has damage immunity 5.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'At the end of each of your turns until the end of the encounter or until you are dying, each target gains 3 surges.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'Each ally the target comes adjacent to during the forced movement can spend a Recovery.'
				),
			],
		}),
	],

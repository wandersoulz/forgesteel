...
export const summoner: HeroClassInterface = {
	id: 'class-summoner',
	name: 'Summoner',
	description: `
				...
				ElementFactory.FeatureFactory.createHeroicResource({
					...
					gains: [
						...
						},
						{
							tag: 'minion-death',
							trigger:
								'The first time each round that any minion dies unwillingly within your Summoner’s Range',
							value: '1',
						},
					],
					details:
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						type: ElementFactory.AbilityTypeFactory.createMain({
							qualifiers: ['can be used as a free strike'],
							freeStrike: true,
						}),
						keywords: [
						...
						sections: [
							...
							ElementFactory.createAbilitySectionPackage('summoner-strike'),
						],
					}),
				}),
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						distance: [ElementFactory.DistanceFactory.createSummoner()],
...
| Storms    | Quintessence                                          |`,
				...
				ElementFactory.FeatureFactory.createMultiple({
					...
					features: [
						...
						ElementFactory.FeatureFactory.createFollower({
							id: 'summoner-6-3b',
							follower: ElementFactory.createFollower(FollowerType.Artisan),
						}),
						ElementFactory.FeatureFactory.createFollower({
							id: 'summoner-6-3c',
							follower: ElementFactory.createFollower(FollowerType.Sage),
						}),
					],
				}),
				ElementFactory.FeatureFactory.createChoice({
...
If they move adjacent to their target, become targeted by an opportunity attack, or stop moving, they explode, dealing 2 damage to one adjacent creature or object and pushing them 1 square. If a target is affected by two or more minions’ explosions, the effects stack. These minions activate no effects upon death, and you gain no essence from their deaths.`),
		...
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter or until you are dying, each minion under your control during the encounter is the target of the following effect:The target’s strikes can inflict I < [weak] taunted (EoT) to enemies. The potency increases by 1 for each minion that joined the strike.'
				),
			],
		}),
		ElementFactory.createAbility({
...
These minions die at the end of your turn, activate no effects upon death, and you gain no essence from their deaths`),
		...
		ElementFactory.createAbility({
			...
			distance: [ElementFactory.DistanceFactory.createMelee(), ElementFactory.DistanceFactory.createSummoner()],
...
Each target can immediately make each of their saving throws and stand up from prone.`),
		...
		ElementFactory.createAbility({
			...
			distance: [ElementFactory.DistanceFactory.create({ type: AbilityDistanceType.Cube, value: 4, within: 1 })],
			target: 'All enemies and objects',
			cost: 9,
			sections: [
				ElementFactory.createAbilitySectionRoll(
...
You can change the damage type to be a type that your champion deals on their stat block (see Portfolio Champion). For each enemy reduced to 0 Stamina by this ability, an ally within distance can move up to their speed.`),
	...
	subclasses: [circleOfBlight, circleOfGraves, circleOfSpring, circleofStorms],
	level: 1,
	characteristics: [],
};

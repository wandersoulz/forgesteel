...
export const auteur: SubClassInterface = {
	id: 'u3JAwbvGQEHPi6cY',
	name: 'Auteur',
	description:
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						target: 'Special',
						sections: [
							ElementFactory.createAbilitySectionText(` Choose one of the following effects:
...
* You insult a foe where they’re most vulnerable. One enemy within distance takes a bane on the next power roll they make before the end of their next turn.`),
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'An edge on the triggering roll becomes a bane, or a double edge becomes an edge. A bane becomes an edge, or a double bane becomes a bane.'
							),
							ElementFactory.createAbilitySectionSpend({
								value: 3,
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'Until the end of the encounter, each target gains the speed bonus, weapon distance bonus, disengage bonus, and stability bonus of your currently equipped kit in addition to their own kit’s bonuses.'
										),
									],
								}),
							}),

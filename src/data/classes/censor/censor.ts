...
export const censor: HeroClassInterface = {
	id: 'class-censor',
	name: 'Censor',
	description: `
...
As a censor, you’re at your best against the strongest foes. Your judgment terrifies heretics, stops enemies in their tracks, and even hurls them across the battlefield.`,
	...
	subclassName: 'Order',
	subclassCount: 1,
	primaryCharacteristicsOptions: [[Characteristic.Might, Characteristic.Presence]],
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						target: 'One enemy',
						sections: [
							ElementFactory.createAbilitySectionText(`The target is judged by you until the end of the encounter, you use this ability again, you willingly end this effect (no action required), or another censor judges the target.
...
You can choose only one free triggered action option at a time, even if multiple options are triggered by the same effect.`),
							ElementFactory.createAbilitySectionPackage('censor-judgment'),
						],
					}),
				}),
...
Virtue remains until you spend it.`,
	...
	abilities: [
		ElementFactory.createAbility({
			...
			distance: [ElementFactory.DistanceFactory.create({ type: AbilityDistanceType.Cube, value: 2, within: 1 })],
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'Until the start of your next turn, enemies have a bane on ability rolls made against you and each ally adjacent to you.'
				),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'Each enemy frightened by this ability is pushed 2 squares away from the target and takes psychic damage equal to your Presence score.'
				),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			distance: [ElementFactory.DistanceFactory.create({ type: AbilityDistanceType.Aura, value: 2 })],
			target: 'Each enemy in the area',
			cost: 7,
			sections: [
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter or until you are dying, each target takes holy damage equal to your Presence score at the end of each of your turns. A target takes an extra 2d6 holy damage if they are judged by you or if they are adjacent to any enemy.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter or until you are dying, whenever a target uses an ability that costs Malice, they take holy damage equal to three times your Presence score. A target judged by you takes an extra 2d6 holy damage.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter or until you are dying, whenever a target makes a strike, they take holy damage equal to twice your Presence score. A target judged by you takes an extra 2d6 holy damage.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter or until you are dying, whenever a target moves or is force moved out of the area, they take holy damage equal to twice your Presence score. A target judged by you who moves willingly takes an extra 2d6 holy damage.'
				),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'A target can end one effect on them that is ended by a saving throw or that ends at the end of their turn, or a prone target can stand up.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter, whenever any ally deals damage to a target judged by you, that ally gains 1 surge.'
				),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'At the end of each of your turns, a target weakened this way deals holy damage equal to twice your Presence score to each enemy within 2 squares of them. Additionally, a target weakened this way can’t be targeted by their allies’ abilities.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'Until the end of the encounter, while the target is judged by you, you can choose to make them the source of any of your abilities. Additionally, the target counts as an ally for the purpose of flanking.'
				),
			],
		}),
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'At the end of each of your turns, a target dazed this way deals holy damage equal to twice your Presence score to each enemy within 2 squares of them.'
				),
			],
		}),
		ElementFactory.createAbility({

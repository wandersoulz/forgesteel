...
export const oracle: SubClassInterface = {
	id: 'censor-sub-2',
	name: 'Oracle',
	description:
		'Corruption has deep tendrils that can be missed, leading you to specialize in uncovering clandestine threats to your order.',
	featuresByLevel: [
		{
			level: 1,
			features: [
				...
				ElementFactory.FeatureFactory.createPackageContent({
					...
					description:
						'The first time on a turn that you use your Judgment ability to judge a creature, you can deal holy damage equal to twice your Presence score to the judged creature.',
					tag: 'censor-judgment',
				}),
			],
		},
							...
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									type: ElementFactory.AbilityTypeFactory.createTrigger(
										'An enemy within 10 squares starts their turn.'
									),
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'You can spend a Recovery to allow the target to regain Stamina equal to your recovery value. The target can then take their turn immediately before the triggering enemy.'
										),
									],
								}),
							}),
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'The target can use a free triggered action to use a strike signature ability or a strike heroic ability, and has a double edge on that ability. If a heroic ability is chosen, reduce its Heroic Resource cost by 3 (to a minimum cost of 0).'
										),
									],
								}),
							}),
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'Until the end of the encounter or until you are dying, whenever any target takes a triggered action or a free triggered action, that action is negated and the target takes holy damage equal to your Presence score.'
										),
									],
								}),
							}),
					...
					options: [
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'The target obtains a tier 1 or tier 3 outcome on their power roll (your choice). You can then choose another target within distance, who obtains the opposite outcome on their next power roll.'
										),
									],
								}),
							}),
						...
						},
						{
							feature: ElementFactory.FeatureFactory.createAbility({
								ability: ElementFactory.createAbility({
									...
									type: ElementFactory.AbilityTypeFactory.createTrigger(
										'You or another hero ends their turn.'
									),
									keywords: [AbilityKeyword.Magic, AbilityKeyword.Ranged],
									distance: [ElementFactory.DistanceFactory.createRanged(10)],
									target: 'One ally',
									...
									sections: [
										ElementFactory.createAbilitySectionText(
											'The target takes their turn after the triggering hero, and immediately removes all conditions and negative effects on themself. During their turn, the target has a double edge on power rolls.'
										),
									],
								}),
							}),

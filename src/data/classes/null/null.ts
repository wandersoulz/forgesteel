...
export const nullClass: HeroClassInterface = {
	id: 'class-null',
	name: 'Null',
	description: `
...
*"Any weapon can be turned against the hand that wields it."* - Ardashir`,
	...
	subclassCount: 1,
	primaryCharacteristicsOptions: [[Characteristic.Agility, Characteristic.Intuition]],
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						distance: [ElementFactory.DistanceFactory.create({ type: AbilityDistanceType.Aura, value: 1 })],
						target: 'All enemies',
						sections: [
...
This ability remains active even after an encounter ends. It ends only if you are dying or if you willingly end it (no action required)`),
							ElementFactory.createAbilitySectionPackage('null-field'),
						],
					}),
				}),
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						target: 'Self',
						sections: [
							ElementFactory.createAbilitySectionText('You halve the damage.'),
							...
							ElementFactory.createAbilitySectionPackage('inertial-shield'),
						],
					}),
				}),
				...
				ElementFactory.FeatureFactory.createMultiple({
					...
					features: [
						...
						ElementFactory.FeatureFactory.createPackageContent({
							...
							name: 'Psionic Martial Arts',
							description:
								'You use Intuition instead of Might for the power roll and for determining if you can target creatures larger than you.',
						...
						ElementFactory.FeatureFactory.createPackageContent({
							...
							name: 'Psionic Martial Arts',
							description:
								'You use Intuition instead of Might for the power roll and you can choose to slide the target instead of pushing them.',
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						name: 'Reorder',
						description: '',
						type: ElementFactory.AbilityTypeFactory.createTrigger('You start your turn.', { free: true }),
						distance: [ElementFactory.DistanceFactory.createSelf()],
						target: 'Self',
						sections: [
							ElementFactory.createAbilitySectionText(
								'End one effect on you that is ended by a saving throw or that ends at the end of your turn. Alternatively, you can grant this benefit to one creature in the area of your Null Field ability.'
							),
						],
					}),
				}),
	...
	abilities: [
		...
		ElementFactory.createAbility({
			...
			sections: [
				...
				ElementFactory.createAbilitySectionText(
					'You can shift up to half your speed before or after you make the strike.'
				),
			],
		}),
		ElementFactory.createAbility({
		...
		ElementFactory.createAbility({
			...
			type: ElementFactory.AbilityTypeFactory.createTrigger('The target moves or uses a main action', {
				free: true,
			}),
			keywords: [AbilityKeyword.Melee, AbilityKeyword.Psionic, AbilityKeyword.Strike, AbilityKeyword.Weapon],
			distance: [ElementFactory.DistanceFactory.createMelee()],

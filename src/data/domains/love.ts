...
export const love: DomainInterface = {
	id: 'domain-love',
	name: 'Love',
			...
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						cost: 9,
						sections: [
							ElementFactory.createAbilitySectionText('Each target gains 3 of their Heroic Resource.'),
						],
					}),
				}),
			],
		},
		{
			...
			features: [
				ElementFactory.FeatureFactory.create({
					...
					description:
						'You can maintain bonds with up to three willing creatures using your Invocation of the Heart feature.',
				}),
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						id: 'domain-love-7-2',
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'Each target is teleported to unoccupied spaces within 5 squares of a willing creature who you are bonded to with your Invocation of the Heart feature. You don’t need line of effect to the bonded creature but you must be on the same world.'
							),
						],
					}),
				}),
	...
	resourceGains: [
		{
			...
			tag: '',
			trigger:
				'The first time in an encounter that you or any ally within 10 squares uses the Aid Attack maneuver or an ability that targets an ally',
			value: '2',
		},
	],
	defaultFeatures: [
		ElementFactory.FeatureFactory.createPackageContent({
			...
			description:
				'Each ally within 10 squares of you gains temporary Stamina equal to 2 times your Intuition score.',
			tag: 'conduit-prayer',
		}),
	],
};

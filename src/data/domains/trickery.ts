...
export const trickery: DomainInterface = {
	id: 'domain-trickery',
	name: 'Trickery',
			...
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'Each creature in the area has a double edge on tests made to hide or sneak. The aura lasts until you end it (no action required) or until a target harms or deals damage to a creature or object.'
							),
						],
					}),
				}),
			...
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'Each target is invisible until the start of your next turn.'
							),
						],
					}),
				}),
	...
	resourceGains: [
		{
			...
			tag: '',
			trigger:
				'The first time in an encounter that you or a creature within 10 squares takes the Aid Attack or Hide maneuver',
			value: '2',
		},
	],
	defaultFeatures: [
		ElementFactory.FeatureFactory.createPackageContent({
			...
			description:
				'You slide one creature within 10 squares of you up to a number of squares equal to 5 + your conduit level.',
			tag: 'conduit-prayer',
		}),
	],
};

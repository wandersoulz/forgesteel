...
export class MontageData {
	static fightFire: MontageInterface = {
		id: 'montage-fight-fire',
		name: 'Fight Fire',
		description:
			'Fire has broken out in the town! The heroes must prevent the conflagration from spreading while saving as many townsfolk as possible. Their efforts might be more complicated if the cause of the fire — such as a marauding dragon or an invading army — is still around causing trouble.',
		...
		sections: [
			{
				...
				description: '',
				challenges: [
					ElementFactory.createMontageChallenge({
						id: 'bucket-chains',
						name: 'Bucket Chains',
						description:
							'Organize the would-be firefighters into disciplined bucket brigades or fight the fire in some other way.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Prevent the fire from spreading by clearing the ground of flammable materials, either by moving it or burning it away. A creature loses a Recovery if they incur a consequence on this test.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Save people trapped in burning buildings. Heroes can attempt this task twice during the montage, since there are plenty of people to save. A creature that doesn’t have fire immunity loses a Recovery if they incur a consequence on their test.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Find groups that aren’t fighting the fire, such as fleeing civilians, and convince them to help.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Shifting burning debris blocking doorways to allow people to escape the blaze. A creature that doesn’t have fire immunity loses a Recovery if they incur a consequence on this test.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'(if **Free the Horses** was successful) Put the horses to work clearing rubble or bringing people to safety.',
				...
				twists: [
					ElementFactory.createMontageChallenge({
						...
						description:
							'While a hero is in or near a blazing building, it starts to collapse. The hero must escape before the building crumbles.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'At the end of the first round of the montage test, the hostile cause of the fire appears — a squad of an invading army, a dragon, a team of arsonists, and so forth. The characters must engage in a standard or hard encounter with this threat',
					}),
					ElementFactory.createMontageChallenge({
						id: 'help',
						name: 'Help!',
						description:
							'Townsfolk are about to run into a burning building to save a trapped relative. This twist requires two tests, each of which nets a success or a failure for the montage test. One hero can try to prevent the townspeople from entering the burning building while another rescues the relative.',
						characteristics: [Characteristic.Might, Characteristic.Presence],
		...
		outcomes: {
			totalSuccess:
				'The fire is extinguished. Buildings are damaged but no lives were lost. The party achieves 2 Victories if the montage test was hard, or 1 Victory if it was easy or moderate, in addition to any Victories earned from combat during the montage test.',
			partialSuccess:
				'The fire is quenched, although many buildings burned and a few lives were lost. Each character earns 1 Victory if the montage test was moderate or hard, in addition to any Victories earned from combat during the montage test.',
			totalFailure:
				'When the fire finally burns out, the town lies in ruins. Townsfolk mourn their dead or grimly prepare to find a new home. Characters earn no Victories from the montage test, but might earn Victories from combat undertaken during the montage test',
		},
	};

	static infiltrateThePalace: MontageInterface = {
		...
		sections: [
			{
				...
				challenges: [
					ElementFactory.createMontageChallenge({
						...
						name: 'Bribe Guards',
						description:
							'The heroes can pay off guards to look the other way. If successful, one or more heroes’ Wealth is lowered by 1.',
						characteristics: [Characteristic.Presence],
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Researching secret entrances and little-known passageways can be undertaken in forgotten libraries or well-guarded town halls.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Scouting around or consulting contacts can reveal a forgotten back door or accessible window',
						characteristics: [Characteristic.Agility, Characteristic.Intuition],
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'By keeping their ears and eyes open, characters can learn when guards go off duty.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'By procuring forged documents or badges, characters can prepare to walk into the palace in plain sight.',
						characteristics: [Characteristic.Presence, Characteristic.Reason],
...
The first time any hero fails a test made as part of the montage test while the alarm level is 2, they encounter guards and must engage in a hard combat encounter. The second time any hero fails such a test while the alarm level is 2, the montage test is a total failure.`,
				challenges: [
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Avoid Traffic',
						description:
							'By finding the dustiest, least-traveled areas and sticking to them, characters can avoid notice.',
						characteristics: [Characteristic.Intuition, Characteristic.Reason],
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Lay Low',
						description:
							'Once while the alarm level is greater than 0, the heroes can find a place to hide for a bit, reducing the alarm level by 1. This activity doesn’t require a test or generate a success or failure.',
					}),
					ElementFactory.createMontageChallenge({
						id: 'make-a-diversion',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Keeping out of sight is the simplest way for characters to move through the palace.  The heroes can attempt this challenge twice during the montage test.',
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Pose as Guards',
						description:
							'Using stolen or specially prepared uniforms can let the characters move freely through the palace. The test for this challenge gains an edge if the characters prepared disguises in advance (including succeeding on the Use False Identities challenge) or defeated guards during their infiltration',
		...
		outcomes: {
			totalSuccess:
				'The heroes reach their goal and secure an escape route that lets them leave the palace safely. Each character earns 2 Victories if the montage test was hard, or 1 Victory if it was easy or moderate, in addition to any Victories earned from combat during the montage test.',
			partialSuccess:
				'The heroes reach their goal, but they need to fight a standard combat encounter to escape the palace. Each character earns 1 Victory if the montage test was moderate or hard, in addition to any Victories earned from combat during the montage test.',
			totalFailure:
				'The palace is locked down and the heroes’ goal is out of reach. The characters need to fight a hard combat encounter to escape. Characters earn no Victories from the montage test, but might earn Victories from combat undertaken during the montage test.',
		},
	};

	static prepareForBattle: MontageInterface = {
		id: 'montage-prepare-for-battle',
		name: 'Prepare for Battle',
		description:
			'Whether it’s a village threatened by bandits or a great city preparing for a siege, enemies are on their way and readyu to attack. The heroes have a limited time to fortify the settlement’s defenses and bolster its troops.',
		difficulty: EncounterDifficulty.Standard,
		...
		sections: [
			{
				...
				challenges: [
					ElementFactory.createMontageChallenge({
						...
						name: 'Arms and Armor',
						description:
							'Crafting or repairing weapons and armor of all kinds can help rebuild the defenders’ stores.',
						characteristics: [Characteristic.Might, Characteristic.Reason],
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Inspiration',
						description:
							'Improving morale with rousing speeches or performances can help prepare the locals for the fight to come.',
						characteristics: [Characteristic.Intuition, Characteristic.Presence],
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Propaganda',
						description:
							'Characters can attempt to sow confusion or rebellion in the ranks of the approaching army.',
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Stockpiling',
						description:
							'Characters can hunt, forage, or supernaturally conjure food or water to augment the settlement’s supplies.',
						characteristics: [Characteristic.Agility, Characteristic.Reason],
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Trapmaking',
						description:
							'Digging concealed pits, placing hindrances, and setting up ambushes will make it harder for the invaders to approach the settlement.',
						characteristics: [Characteristic.Might, Characteristic.Reason],
						skills: 'Conceal Object, Endurance, Mechanics',
					}),
				],
				twistInfo:
					'At the end of the first round of the montage test, a fast-moving enemy vanguard attack before the settlement’s defenders are ready. The heroes must must engage in an easy combat encounter.',
				twists: [],
			},
		],
		outcomes: {
	...
	static trackTheFugitive: MontageInterface = {
		...
		sections: [
			{
				...
				challenges: [
					ElementFactory.createMontageChallenge({
						...
						name: 'Ask Around',
						description:
							'Characters can gather clues from locals or bystanders — or if they have the proper magic, from animals or the dead.',
						characteristics: [Characteristic.Intuition, Characteristic.Presence],
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Looking for tracks or other signs of the fugitive’s passage can lead the characters on.',
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Characters can climb up high to get the big picture of where the fugitive might have gone.',
						characteristics: [Characteristic.Agility, Characteristic.Might],
						skills: 'Climb, Gymnastics, Jump',
					}),
					ElementFactory.createMontageChallenge({
						...
						description:
							'The heroes might have an idea where the quarry is headed. A character gains an edge on the test for this challenge if they know the quarry well.',
						characteristics: [Characteristic.Intuition, Characteristic.Reason],
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Push Ahead',
						description:
							'While the quarry is resting, the heroes have a chance to close in. The hero making the test for this challenge loses a Recovery.',
						characteristics: [Characteristic.Might],
		...
		outcomes: {
			...
			partialSuccess:
				'If the quarry was trying to evade capture, they reach their destination. They find allies and a fortified position from which to defend themselves, or they might have time to cause more harm. If the quarry was lost or kidnapped, they are grievously injured when found. Each character earns 1 Victory if the montage test was moderate or hard, in addition to any Victories earned from combat during the montage test.',
			totalFailure:
				'The trail has gone cold, and the heroes will need to seek fresh clues or a different approach before they can resume the hunt. Characters earn no Victories from the montage test, but might earn Victories from combat undertaken during the montage test.',
		},
	};

	static wildernessRace: MontageInterface = {
		...
		sections: [
			{
				...
				challenges: [
					ElementFactory.createMontageChallenge({
						...
						name: 'Avoid Hazards',
						description:
							'Characters can determine ways to overcome the natural hazards of the wilderness, such as finding insect-repelling herbs in a swamp or making snowshoes to cross tundra.',
						characteristics: [Characteristic.Intuition, Characteristic.Reason],
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Carry Baggage',
						description:
							'By carrying supplies for the weaker party members, characters can increase the whole party’s speed.',
						characteristics: [Characteristic.Might],
						skills: 'Endurance, Lift',
					}),
					ElementFactory.createMontageChallenge({
					...
					ElementFactory.createMontageChallenge({
						...
						description:
							'Characters can keep up the party’s morale during a forced march with cheer and song.',
						characteristics: [Characteristic.Presence],
					...
					ElementFactory.createMontageChallenge({
						...
						name: 'Scout Ahead',
						description:
							'Investigating the path ahead lets the characters avoid dead-ends and arduous terrain',
						characteristics: [Characteristic.Agility, Characteristic.Intuition],
		...
		outcomes: {
			totalSuccess:
				'The heroes reach their goal in time. Each character earns 2 Victories if the montage test was hard, or 1 Victory if it was easy or moderate, in addition to any Victories earned from combat during the montage test.',
			partialSuccess:
				'To reach their goal in time, the heroes must sprint over the last leg of the journey, with each character spending 2 Recoveries to do so. (If even one character doesn’t have 2 Recoveries remaining, the characters instead earn a total failure for the montage test.) Each character earns 1 Victory if the montage test was moderate or hard, in addition to any Victories earned from combat during the montage test.',
			totalFailure:
				'The heroes don’t arrive in time to avert catastrophe. Characters earn no Victories from the montage test, but might earn Victories from combat undertaken during the montage test.',
		},
	};
}

...
export const ashenHoarder: MonsterGroupInterface = {
...
When an ashen hoarder obtains corpses, they impale them on the many sharp bones protruding from their back. These spikes secure the corpses and charge them with corruptive energy, ensuring the cargo is ready for experimentation when it arrives at the necromancer’s door.`,
	...
	malice: [
		ElementFactory.FeatureFactory.createMalice({
			...
			sections: ['The ashen hoarder moves up to their speed and can make a free strike against two targets.'],
		}),
		ElementFactory.FeatureFactory.createMalice({
	...
	monsters: [
		ElementFactory.createMonster({
			...
			stability: 3,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(4, -2, -2, 0, -5),
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						type: ElementFactory.AbilityTypeFactory.createTrigger('The ashen hoarder takes damage.'),
						distance: [ElementFactory.DistanceFactory.createSelf()],
						target: 'Self',
						cost: 2,
						sections: [
				...
				ElementFactory.FeatureFactory.create({
					...
					description:
						'When the ashen hoarder is reduced to 0 Stamina, they explode in a swirling singularity of bone shards and soul energy. Each creature within 5 squares of the ashen hoarder who has <code>M < 3</code> takes 11 corruption damage. A creature killed by this damage has their soul sucked into the vortex and lost somewhere in the Abyssal Wasteland, and can’t be brought back to life until their soul is recovered.',
				}),
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({

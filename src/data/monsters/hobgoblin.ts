...
export const hobgoblin: MonsterGroupInterface = {
...
Devils captured the grack’tanars eons ago. Broken, these demons wait for a call to war, hungry and frothing in the Seven Cities of Hell. Slaughter demons are eager to kill and please their devil captors so they might be sent out again, and they rarely turn on hobgoblins unless they fall into lethe (see the Demons section).`,
	...
	malice: [
		ElementFactory.FeatureFactory.createMalice({
			...
			sections: ['Each goblin in the encounter gains a +3 bonus to speed until the end of the round.'],
		}),
		ElementFactory.FeatureFactory.createMalice({
			...
			cost: 5,
			icon: StatBlockIcon.Trait,
			sections: ['Each hobgoblin in the encounter shifts up to their speed and can take the Defend main action.'],
		}),
		ElementFactory.FeatureFactory.createMalice({
	...
	monsters: [
		ElementFactory.createMonster({
			...
			stability: 0,
			freeStrikeDamage: 2,
			characteristics: ElementFactory.createCharacteristics(0, 1, 2, 0, 3),
					...
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText(
								'The lancer deals an additional 2 damage if they strike the target from 1 or more squares above.'
							),
						],
					}),
				}),
		...
		ElementFactory.createMonster({
			...
			stability: 0,
			freeStrikeDamage: 5,
			characteristics: ElementFactory.createCharacteristics(0, 1, 2, 2, 3),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
		...
		ElementFactory.createMonster({
			...
			stability: 4,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(3, 2, 3, 0, 2),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'If the target has <code>P < 2</code>, they are taunted (EoT). While taunted this way, the target takes 1d6 fire damage whenever they use an ability or strike that doesn’t target the trooper.'
							),
						],
					}),
				}),
		...
		ElementFactory.createMonster({
			...
			stability: 4,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(0, 2, 2, 3, 3),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
		...
		ElementFactory.createMonster({
			...
			stability: 0,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(1, 3, 2, 1, 0),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
		...
		ElementFactory.createMonster({
			...
			stability: 0,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(0, 2, 3, 2, 2),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'The war mage consecrates the area and causes it to smolder until the end of the encounter. The area is difficult terrain and any enemy in the area has fire weakness 10.'
							),
						],
					}),
				}),
		...
		ElementFactory.createMonster({
			...
			stability: 2,
			freeStrikeDamage: 7,
			characteristics: ElementFactory.createCharacteristics(4, 2, 2, 3, 3),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
		...
		ElementFactory.createMonster({
			...
			stability: 3,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(3, 0, -1, 1, 0),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
				...
				ElementFactory.FeatureFactory.create({
					...
					name: 'Drag Below',
					description:
						'The slaughter demon can make a free strike as part of using the Dig maneuver. If the target of the free strike has <code>M < 2</code>, they are grabbed and take a bane on the Escape Grab maneuver.',
				}),
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({

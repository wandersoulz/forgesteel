...
export const human: MonsterGroupInterface = {
...
Humans see unrealized potential everywhere, whether envisioning an untamed forest transformed into a prosper ous village, or an ancient dungeon yielding chests filled with coins. Pursuing such ambitions might end in catastrophe, but for these gamblers, it’s a game worth playing.`,
		},
		{
			...
			description:
				'With an appetite for warfare and gold, human adventurers are well represented in most mer cenary bands. A human mercenary makes a stout ally … if you can afford their price.',
		},
		{
			id: 'human-info-4',
	...
	malice: [
		...
		ElementFactory.FeatureFactory.createMalice({
			...
			cost: 7,
			icon: StatBlockIcon.Trait,
			sections: [' Each non-minion human in the encounter regains Stamina equal to 5 times their level.'],
		}),
	],
	monsters: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText('One creature within 5 squares regains 1 Stamina.'),
						],
					}),
				}),
				ElementFactory.FeatureFactory.create({
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText('The brawler pushes the target up to 5 squares.'),
						],
					}),
				}),
				ElementFactory.FeatureFactory.create({
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						type: ElementFactory.AbilityTypeFactory.createManeuver({ qualifiers: ['1 malice per minion'] }),
						cost: 1,
		...
		ElementFactory.createMonster({
			...
			stability: 2,
			freeStrikeDamage: 4,
			characteristics: ElementFactory.createCharacteristics(3, 2, 2, 0, 2),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
					id: 'human-13-1',
					modifiers: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [ElementFactory.createAbilitySectionText('The damage is halved.')],
					}),
				}),
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
		...
		ElementFactory.createMonster({
			...
			stability: 2,
			freeStrikeDamage: 5,
			characteristics: ElementFactory.createCharacteristics(2, 3, 2, 3, 2),
			features: [
				ElementFactory.FeatureFactory.createDamageModifier({
					id: 'human-14-1',
					modifiers: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						name: 'Bloodstones',
						type: ElementFactory.AbilityTypeFactory.createTrigger('The bandit chief makes a power roll.'),
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [ElementFactory.createAbilitySectionText('Each target makes a ranged free strike.')],
					}),
				}),
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
		...
		ElementFactory.createMonster({
			...
			speed: ElementFactory.createSpeed(5, 'flying'),
			stamina: 30,
			stability: 0,
			freeStrikeDamage: 3,
			characteristics: ElementFactory.createCharacteristics(2, 2, -3, 1, -2),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						name: 'Dive',
						type: ElementFactory.AbilityTypeFactory.createManeuver(),
						sections: [ElementFactory.createAbilitySectionText('The hawk moves up to their speed.')],
					}),
				}),
				ElementFactory.FeatureFactory.create({

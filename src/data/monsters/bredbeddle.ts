...
export const bredbeddle: MonsterGroupInterface = {
...
Travelers warn one another of roads rumored to be haunted by “wild green knights,” often avoiding the routes for months. No journey is worth losing your head over.`,
		},
		{
			...
			description:
				'When a bredbeddle communicates, they usually use the languages that their head’s previous owner knew or High Kuric. A bredbeddle that’s taken multiple heads over their lifetime will most likely know Caelian as well.',
		},
	],
	malice: [
	...
	monsters: [
		ElementFactory.createMonster({
			...
			stability: 4,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(3, 0, -3, 1, 2),
			features: [
...
A creature must succeed on a hard **Might test** made as a maneuver to rip a head oﬀ the bredbeddle. If they fail, the bredbeddle makes a free strike against them.`,
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						distance: [ElementFactory.DistanceFactory.createSelf('while headless')],
						target: 'Self',
						sections: [
							ElementFactory.createAbilitySectionText(
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						target: 'One enemy',
						sections: [
							ElementFactory.createAbilitySectionText(`The bredbeddle points at the target and issues them a challenge. If the target refuses, they turn green until the end of the encounter (see Turn Green).

...
export const basilisk: MonsterGroupInterface = {
...
<code>17+</code> The character creates three doses of salve.`,
			...
			ability: ElementFactory.createAbility({
				...
				sections: [
					ElementFactory.createAbilitySectionText(
						'The basilisk spits up a chuck of a past petrified victim.'
					),
	...
	monsters: [
		ElementFactory.createMonster({
			...
			freeStrikeDamage: 5,
			characteristics: ElementFactory.createCharacteristics(2, 0, -3, -1, -1),
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'Each target takes 5 damage. Any target who has <code>A < 2</code> is also bleeding (save ends).'
							),
						],
					}),
				}),
		...
		ElementFactory.createMonster({
			...
			size: ElementFactory.createSize(2),
			freeStrikeDamage: 4,
			characteristics: ElementFactory.createCharacteristics(1, 2, -3, -1, -1),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'Each target takes 4 acid damage. Any target who has <code>M < 2</code> is also slowed (save ends).'
							),
						],
					}),
				}),
				ElementFactory.FeatureFactory.create({
					...
					description:
						'Any creature who starts their turn adjacent to the tonguesnapper and has <code>M < 1</code> is slowed (save ends).',
				}),
				ElementFactory.FeatureFactory.createDamageModifier({

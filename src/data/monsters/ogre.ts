...
export const ogre: MonsterGroupInterface = {
...
Rumor has it that a cyclops can emit a mile-long energy ray from their eye. However, focusing such power is nearly impossible when their vision is usually too irritated and clouded by dirt.`,
	...
	monsters: [
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText('The goon slides the target up to 5 squares'),
						],
					}),
				}),
		...
		ElementFactory.createMonster({
			...
			stability: 5,
			freeStrikeDamage: 4,
			characteristics: ElementFactory.createCharacteristics(5, -1, -1, 3, -1),
		...
		ElementFactory.createMonster({
			...
			stability: 2,
			freeStrikeDamage: 4,
			characteristics: ElementFactory.createCharacteristics(4, 2, -1, 0, -1),

...
export const countRhodar: MonsterGroupInterface = {
...
How his return might be possible is not clear, but it would surely involve the Rod of Count Rhodar Von Glauer, currently held under the city of Capital in the Great Reliquary of the Last Emperor, also known as Ringwell.`,
	...
	monsters: [
		ElementFactory.createMonster({
			...
			speed: ElementFactory.createSpeed(12, 'fly, hover, teleport'),
			stamina: 650,
			stability: 3,
			freeStrikeDamage: 10,
			characteristics: ElementFactory.createCharacteristics(3, 5, 2, 2, 3),
			features: [
					...
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText(
								'A target restrained this way is impaled by a spear. Rhodar has four spears, each of which can be used to impale a target. At the start can summon any of his spears back to himself, ending the restrained condition on an impaled target.'
							),
						],
					}),
				}),
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						distance: [ElementFactory.DistanceFactory.createSpecial('Two 3 cubes within 5')],
						target: 'Special',
						cost: 2,
						sections: [
							ElementFactory.createAbilitySectionText(
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText(
								'Rhodar teleports to an unoccupied space adjacent to one target after the ability resolves.'
							),
						],
					}),
				}),

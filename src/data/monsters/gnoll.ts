...
As his last act, the demon gave the fiendish hyenas a demonic cunning to match their insatiable hunger, bound his evil will to theirs, and turned their paws into hands so they could wield weapons. He called these new fiends “gnoll,” an Abyssal word that has many meanings, one of which is “to consume completely.” And true to their name, the gnolls immediately devoured their creator.`,
	...
	malice: [
		ElementFactory.FeatureFactory.createMaliceAbility({
			ability: ElementFactory.createAbility({
				...
				distance: [
					ElementFactory.DistanceFactory.create({ type: AbilityDistanceType.Cube, value: 1, within: 3 }),
				],
				target: 'Special',
				sections: [
					ElementFactory.createAbilitySectionText(
						'A gnoll acting this turn drops an iron-jawed snare into the affected area. The first time an enemy steps into the area, they make an **Agility test**. If they were unaware of the snare, they make the roll with a bane.'
					),
					...
					ElementFactory.createAbilitySectionText(
						'While an enemy is bleeding from the snare, each gnoll in the encounter gains an edge on strikes made against them.'
					),
				],
			}),
		}),
	...
	monsters: [
		ElementFactory.createMonster({
			...
			features: [
				...
				ElementFactory.FeatureFactory.create({
					...
					name: 'Death Snap',
					description:
						'When the abyssal hyena is reduced to 0 Stamina, they make a melee free strike before dying.',
				}),
			],
		}),
		ElementFactory.createMonster({
			...
			characteristics: ElementFactory.createCharacteristics(2, 0, 1, 0, -2),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
		...
		ElementFactory.createMonster({
			...
			characteristics: ElementFactory.createCharacteristics(2, 1, -1, 0, 0),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText('The target has a bane on their next power roll.'),
						],
					}),
				}),
				ElementFactory.FeatureFactory.create({
					...
					name: 'Death Frenzy',
					description:
						'Whenever a non-minion ally within 5 squares of the chainflail is reduced to 0 Stamina, the mage mauler can move up to their speed.',
				}),
			],
		}),
		ElementFactory.createMonster({
			...
			stability: 1,
			freeStrikeDamage: 2,
			characteristics: ElementFactory.createCharacteristics(1, 2, 0, 0, -2),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
		...
		ElementFactory.createMonster({
			...
			stability: 1,
			freeStrikeDamage: 2,
			characteristics: ElementFactory.createCharacteristics(1, 0, 0, 2, 2),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'Two **abyssal hyenas** claw up from the ground in unoccupied squares within distance.'
							),
						],
					}),
				}),
			...
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText('Each enemy target makes an **Intuition test**.'),
							ElementFactory.createAbilitySectionRoll(
								ElementFactory.createPowerRoll({
							...
							ElementFactory.createAbilitySectionText(
								"Targets who haven't used a cackletongue maneuver on this turn use it immediately at no cost."
							),
						],
					}),
				}),
		...
		ElementFactory.createMonster({
			...
			stability: 3,
			freeStrikeDamage: 5,
			characteristics: ElementFactory.createCharacteristics(2, -1, -3, 0, -1),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
		...
		ElementFactory.createMonster({
			...
			stability: 1,
			freeStrikeDamage: 5,
			characteristics: ElementFactory.createCharacteristics(3, 3, 0, 0, 3),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'The carnage summons four **abyssal hyenas** and 5 **abyssal hyenas** into unoccupied spaces within distance.'
							),
						],
					}),
				}),
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								"The carnage's eyes and all explosed blood within distance glow bright red. Each target makes a **Presence test**."
							),
							ElementFactory.createAbilitySectionRoll(
								ElementFactory.createPowerRoll({

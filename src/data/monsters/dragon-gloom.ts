...
export const dragonGloom: MonsterGroupInterface = {
...
When a gloom dragon slays a victim, they take not just their valuables, but also whatever is most meaningful to their fears. A broken mirror from a dead lover will always fascinate a gloom dragon more than a perfect ruby brooch.`,
	...
	monsters: [
		ElementFactory.createMonster({
			...
			stability: 2,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(2, 4, 1, 3, 4),
			features: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						type: ElementFactory.AbilityTypeFactory.createTrigger('A creature within 10 squares moves.', {
							free: true,
						}),
						cost: 1,
						...
						sections: [
							ElementFactory.createAbilitySectionText(
								'The dragon moves two existing cubes of magical darkness up to 10 squares each.'
							),
						],
					}),
				}),

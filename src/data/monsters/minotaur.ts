...
export const minotaur: MonsterGroupInterface = {
...
New minotaurs were born, and decades later, hundreds charged out of the maze together. These minotaurs reduced Kham to ruins, then they spread far and wide, fearing reprisal from neighboring lands. Even today, many minotaurs shun civilization, preferring to live in winding canyons, twisting ruins, bewildering dungeons, or other labyrinthine terrain where they can continue to hunt. However, small numbers of minotaurs can be found in nearly any terrain or settlement.`,
	...
	monsters: [
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText('The minotaur shifts up to 3 squares.'),
						],
					}),
				}),
				ElementFactory.FeatureFactory.createAbility({
		...
		ElementFactory.createMonster({
			...
			stability: 2,
			freeStrikeDamage: 6,
			characteristics: ElementFactory.createCharacteristics(2, 1, 0, 2, -1),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
		...
		ElementFactory.createMonster({
			...
			characteristics: ElementFactory.createCharacteristics(4, 2, 0, 3, -1),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
		...
		ElementFactory.createMonster({
			...
			characteristics: ElementFactory.createCharacteristics(3, 4, 0, 1, -1),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText(
								'A target that is force moved into an obstacle is M<3 bleeding (save ends).'
							),
						],
					}),
				}),
		...
		ElementFactory.createMonster({
			...
			characteristics: ElementFactory.createCharacteristics(5, 5, 0, 2, -1),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({

...
export const lizardfolk: MonsterGroupInterface = {
...
- **War Party, 48 EV**: Eight shellguard, eight grunts, eight tonguers, one scaletooth, one skyterror, one bloodeye, one deathrex`,
	...
	monsters: [
			...
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						type: ElementFactory.AbilityTypeFactory.createMain(),
						keywords: [AbilityKeyword.Melee, AbilityKeyword.Strike, AbilityKeyword.Weapon],
						distance: [ElementFactory.DistanceFactory.createMelee(8)],
						target: 'One creature or object per minion',
						cost: 'signature',
						sections: [
							ElementFactory.createAbilitySectionRoll(
		...
		ElementFactory.createMonster({
			...
			size: ElementFactory.createSize(1, 'M'),
			freeStrikeDamage: 3,
			characteristics: ElementFactory.createCharacteristics(1, 1, 0, 2, 0),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							...
							ElementFactory.createAbilitySectionText(
								'The scaletooth needs their tail to use this ability. The scaletooth can’t grapple more than one creature or object with this ability.'
							),
						],
					}),
				}),
		...
		ElementFactory.createMonster({
			...
			freeStrikeDamage: 4,
			characteristics: ElementFactory.createCharacteristics(3, 2, 0, 1, 2),
			features: [
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
				...
				ElementFactory.FeatureFactory.createAbility({
					ability: ElementFactory.createAbility({
						...
						sections: [
							ElementFactory.createAbilitySectionText('The deathrex slides the target up to 5 squares.'),
						],
					}),
				}),
				ElementFactory.FeatureFactory.createAbility({

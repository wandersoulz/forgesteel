...
export class HeroLogic {
		...
		return abilities.map(a => {
			...
			if (customization) {
				...
				if (customization.characteristic) {
					ability.sections.filter((s: any) => s.type === 'roll').forEach((s: any) => s.roll.characteristic = [ customization.characteristic! ]);
				}
	...
	static createRandomHero = async () => {
		...
		hero.career.incitingIncidents.selected = CoreUtils.copy(Collections.draw(hero.career.incitingIncidents.options));

import { Sourcebook, SourcebookElementKind } from '@/core/models/sourcebook';
import { Adventure } from '@/core/models/adventure';
import { AdventureLogic } from '@/core/logic/adventure-logic';
...
import { MonsterFilter } from '@/core/models/filter';
	...
	static getAdventures = (sourcebooks: Sourcebook[], searchTerm: string) => {
		try {
			return SourcebookLogic.getAdventures(sourcebooks)
				.filter(item => CoreUtils.textMatches([
					item.name,
					item.description
				], searchTerm));
		} catch (ex) {

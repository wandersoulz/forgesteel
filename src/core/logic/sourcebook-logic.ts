...
type SourcebookLoader = () => Promise<{ [key: string]: Sourcebook }>;

export class SourcebookLogic {
	static registry: Record<string, SourcebookLoader> = {
		'core': () => import('../../data/sourcebooks/official/core'),
		'orden': () => import('../../data/sourcebooks/official/orden'),
		'community': () => import('../../data/sourcebooks/community/community'),
		'magazine-blacksmith': () => import('../../data/sourcebooks/third-party/magazine-blacksmith'),
		'magazine-ratcatcher': () => import('../../data/sourcebooks/third-party/magazine-ratcatcher'),
		'triglav': () => import('../../data/sourcebooks/third-party/triglav'),
	};

	static async getSourcebooks(activeIDs: string[], homebrew: Sourcebook[] = []): Promise<Sourcebook[]> {
		const promises = activeIDs
			.filter(id => this.registry[id])
			.map(async id => {
				const module = await this.registry[id]();
				return Object.values(module)[0] as Sourcebook;
			});

		const sourcebooks = await Promise.all(promises);
		sourcebooks.push(...homebrew);

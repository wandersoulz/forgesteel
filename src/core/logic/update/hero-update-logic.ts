...
export class HeroUpdateLogic {
	...
	static updateHeroStructure = (hero: Hero, sourcebooks: Sourcebook[]) => {
		...
		if (hero.settingIDs === undefined) {
			hero.settingIDs = Object.keys(SourcebookLogic.registry);
		}

		hero.settingIDs = hero.settingIDs;
		...
		if (hero.career) {
			...
			if (hero.career.incitingIncidents.selected === undefined) {
				...
				if (current) {
					hero.career.incitingIncidents.selected = CoreUtils.copy(current);
				}
			}
	...
	static updateHeroData = (hero: Hero, sourcebooks: Sourcebook[]) => {
		const original = CoreUtils.copy(hero);
		...
		try {
			if (original.ancestry) {
				...
				if (ancestry) {
					hero.ancestry = CoreUtils.copy(ancestry);
				}
			}
		} catch (ex) {
			// console.error(ex);
		...
		try {
			if (original.culture) {
				...
				if (culture) {
					hero.culture = CoreUtils.copy(culture);
				}
		...
		try {
			if (original.career) {
				...
				if (career) {
					hero.career = CoreUtils.copy(career);
		...
		try {
			if (original.class) {
				...
				if (heroClass) {
					hero.class = CoreUtils.copy(heroClass);
					...
					// Update subclass
					hero.class!.subclasses.forEach(sc => {
						const originalSubClass = original.class!.subclasses.find((s: any) => s.id === sc.id);
						if (originalSubClass) {
		...
		try {
			if (original.complication) {
				...
				if (complication) {
					hero.complication = CoreUtils.copy(complication);
				}
			}
		} catch (ex) {
			// console.error(ex);
			...
			switch (feature.type) {
				...
				case FeatureType.Domain: {
					...
					feature.data.selected = SourcebookLogic.getDomains(sourcebooks)
						...
						.map(d => {
							const copy = CoreUtils.copy(d);
							copy.featuresByLevel = copy.featuresByLevel.filter((lvl: any) => feature.data.levels.includes(lvl.level));
							[ ...copy.defaultFeatures, ...copy.featuresByLevel.flatMap((lvl: any) => lvl.features) ].forEach(f => FeatureLogic.switchFeatureCharacteristic(f, Characteristic.Intuition, feature.data.characteristic));
							return copy;
						});
					break;
				}
				case FeatureType.DomainFeature: {
					...
					HeroLogic.getDomains(hero).forEach(d => {
						d.featuresByLevel
							.filter((lvl: any) => lvl.level === feature.data.level)
							.forEach((lvl: any) => domainFeatures.push(...lvl.features));
					});
				...
				case FeatureType.TitleChoice: {
					...
					feature.data.selected = oFeature.data.selected.map(oTitle => {
						...
						if (title) {
							const copy = CoreUtils.copy(title);

...
export class ModifierLogic {
	static calculateModifierValue = (modifier: ModifierInterface, creature: Hero | Monster) => {
		let value = 0;
		...
		if (modifier.valueCharacteristics.length > 0) {
			const characteristicValue =
				Collections.max(
					modifier.valueCharacteristics.map((ch) => CreatureLogic.getCharacteristic(creature, ch)),
					(v) => v
				) || 0;

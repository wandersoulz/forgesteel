...
import {
	FollowerSheetInterface,
	ItemSheetInterface,
	ProjectSheetInterface,
} from '../../models/classic-sheets/hero-sheet';
...
import { FeatureInterface, FeatureClassAbilityInterface } from '../../models/feature';
...
import { RulesItemInterface } from '../../models/rules-item';
...
export class SheetFormatter {
	// #region Text & FeatureInterface Manipulation
	...
	static convertFeaturesShort = (features: FeatureInterface[]): FeatureInterface[] => {
		return this.convertFeatures(features.map((f) => ({ feature: f, display: 'short' })));
	};

	static convertFeatures = (
		features: { feature: FeatureInterface; display: 'short' | 'full' }[]
	): FeatureInterface[] => {
		features.sort((a, b) => this.sortFeatures(a.feature, b.feature));
		const results: FeatureInterface[] = [];
	...
	static cleanupFeature = (feature: FeatureInterface, display: 'short' | 'full' = 'short'): FeatureInterface => {
		const result = CoreUtils.copy(feature);
	...
	static fixClassAbilityNames = (feature: FeatureInterface, abilities: AbilityInterface[]) => {
		let result = feature;
		if (result.type === FeatureType.ClassAbility) {
			result = CoreUtils.copy(result) as FeatureClassAbilityInterface;
	...
	static enhanceFeatures = (features: FeatureInterface[]): FeatureInterface[] => {
	...
	static enhanceFeature = (feature: FeatureInterface): FeatureInterface => {
		const result = CoreUtils.copy(feature);
	...
	// #endregion Text & FeatureInterface Manipulation

	// #region FeatureInterface Logic
	static isLongFeature = (f: FeatureInterface, check: number = 2): boolean => {
		return (
			[FeatureType.Text, FeatureType.Package].includes(f.type) && f.description.trim().split('\n').length > check
		);
	};
	...
	static isVERYLongFeature = (f: FeatureInterface): boolean => {
	...
	static divideFeatures = (
		sourcedFeatures: { feature: FeatureInterface; source: string }[],
		hero: Hero | null,
		availableSpace: number,
		lineLength: number = 50,
		columns: number = 1
	) => {
		const displayed: { feature: FeatureInterface; display: 'short' | 'full' }[] = [];
		const reference: { feature: FeatureInterface; source: string }[] = [];
		...
		sourcedFeatures.forEach((sf) => {
			const sourcedFeature = CoreUtils.copy(sf);
				...
				if (this.isLongFeature(f)) {
					...
					if (this.containsExtractableReferenceContent(f)) {
						...
						if (referenceContent) {
							...
							if (!referenceContent.leftover.length) return;
					...
					if (addToReference) reference.push(sourcedFeature);
		...
		if (size < availableSpace) {
			const referenceWithSize = distinctReference
				.map((sf) => {
	...
	static containsExtractableReferenceContent = (feature: FeatureInterface): boolean => {
		return this.containsLargeTable(feature) || this.isSpecialHandlingFeature(feature);
	};

	static containsLargeTable = (feature: FeatureInterface): boolean => {
	...
	static isSpecialHandlingFeature = (feature: FeatureInterface): boolean => {
		return ['beastheart-1-2b', 'summoner-1-2'].includes(feature.id);
	};

	static extractReferenceContent = (
		feature: FeatureInterface
	): { title: string; content: string; leftover: string; wide: boolean; section: string } | null => {
	...
	static extractTable = (
		...
		const tableAndLabel = text.match(
			/(?:^\s*(?:\*\*|#+\s+)(?<label>[^*]*)(?:\*\*)?(?:\n|$)+)?(?<table>(?:^\s*\|(?:.*\|){2,}\s*(?:\n|$)){3,})/m
		);
	...
	static extractSpecialHandlingData = (
		feature: FeatureInterface
	): { title: string; content: string; leftover: string; wide: boolean; section: string } | null => {
		switch (feature.id) {
			case 'summoner-1-2': {
	...
	static formatAbilityTier = (
		value: string,
		tier: number,
		ability: Ability,
		creature: Hero | Monster | undefined
	) => {
		if (ability.distance.length > 1) {
			...
			const values = distanceTypes.map((d) => {
				return {
					type: d,
					effect: SheetFormatter.cleanupText(ability.getTierEffectCreature(value, tier, d, creature)).split(
						'; '
					),
				};
			});
		...
		return SheetFormatter.cleanupText(ability.getTierEffectCreature(value, tier, undefined, creature));
	};
	// #endregion FeatureInterface Logic

	// #region AbilityInterface Logic
	static abilitySections = (
		sections: (
			| AbilitySectionTextInterface
			| AbilitySectionFieldInterface
			| AbilitySectionRollInterface
		...
		creature: Hero | MonsterInterface | undefined
	): string => {
		const lines: string[] = [];
	...
	// #endregion AbilityInterface Logic

	// #region Size Calculation
	static calculateFeatureSize = (
		f: FeatureInterface,
		hero: Hero | null,
		lineWidth: number,
		countShortenedText: boolean = true
	): number => {
		let size = 1;
	...
	static calculateFeatureReferenceSize = (
		features: { feature: FeatureInterface; source: string }[],
		hero: Hero,
		lineWidth: number,
		columns: number
	): number => {
	...
	static calculateInventorySize = (items: ItemSheetInterface[], lineWidth: number): number => {
	...
	static calculateItemSize = (item: ItemSheetInterface, lineWidth: number): number => {
		let size = 2;
		if (item.features) {
	...
	static calculateTitlesSize = (titles: TitleInterface[] | undefined, lineWidth: number): number => {
		let size = 2.5; // Card header
	...
	static calculateFollowerSize = (follower: FollowerSheetInterface, lineWidth: number): number => {
		let size = 0;
	...
	static calculateMonsterSize = (monster: MonsterSheetInterface, lineWidth: number, columns: number = 1): number => {
		let size = 0;
	...
	static calculateFollowersSize = (followers: FollowerSheetInterface[], lineWidth: number): number => {
		let size = 2.5; // card header
		followers.forEach((f) => {
	...
	// COMPACT AbilityInterface display - e.g. for Retainers & Monsters
	static calculateAbilityComponentSize = (ability: AbilitySheetInterface, lineWidth: number): number => {
		...
		if (ability.trigger) size += this.countLines(ability.trigger, lineWidth);
	...
	static calculateRuleReferenceCardSize = (rule: RulesItemInterface, lineWidth: number): number => {
		let size = 3;
		size += this.countLines(rule.content, lineWidth - 4); // account for padding
		return size;
	};

	static calculateProjectDetailCardSize = (project: ProjectSheetInterface, lineWidth: number): number => {
		let size = 3;
		size += this.calculateProjectComponentSize(project, lineWidth);
		return size;
	};

	static calculateProjectComponentSize = (project: ProjectSheetInterface, lineWidth: number): number => {
		let size = 0;
	...
	static calculateProjectsOverviewCardSize = (projects: ProjectSheetInterface[], lineWidth: number): number => {
		let size = 2.7;
		projects.forEach((p) => {
			size += 1.8; // header
	...
	static calculateAbilitySize = (ability: AbilitySheetInterface | undefined, lineWidth: number): number => {
		let size = 0;
		...
		if (ability) {
			...
			if (ability.trigger) size += 1 + this.countLines(ability.trigger, lineWidth);
			if (ability.effect) {
				if (ability.hasPowerRoll) {
					size += 0.5; // extra padding when effect follows power roll
				}
			...
			text
				...
				.reduce((n, l) => {
					...
					if (l.startsWith('|:---')) {
						...
						// bolded label - will have extra bottom margin
						len += 0.5;
					} else if (l.startsWith('* ')) {
						// list item, will be indented
	...
	static getLargestSize = (abilities: AbilitySheetInterface[], lineLength: number): number => {
	...
	static sortFeatures = (a: FeatureInterface, b: FeatureInterface): number => {
	...
	static keywordOrder: string[] = ['Melee', 'Ranged', 'Magic'];
	...
	static sortAbilitiesByLength = (
		a: AbilitySheetInterface,
		b: AbilitySheetInterface,
		order: 'asc' | 'desc' = 'asc'
	): number => {
	...
	static characteristicOrder: string[] = ['M', 'A', 'R', 'I', 'P'];
	...
	static resourceGainTriggerOrder: string[] = ['when', 'the first time'];

	static sortHeroicResourceGains = (
		a: { tag: string; trigger: string; value: string },
		b: { tag: string; trigger: string; value: string }
	): number => {

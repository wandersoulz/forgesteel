	...
	static getCustomizedMonster = (monsterID: string, customization: EncounterSlotCustomization, sourcebooks: Sourcebook[]) => {
		...
		if (monster && monsterGroup) {
			...
			customization.itemIDs.forEach(id => {
				...
				if (item) {
					...
					item.featuresByLevel
						...
						.forEach(f => {
							const featureCopy = CoreUtils.copy(f);
			...
			if (customization.levelAdjustment !== 0) {
				...
				const max = Collections.max(copy.characteristics.map((c: any) => c.value), (v: any) => v) as number;
				copy.characteristics.filter((c: any) => c.value === max).forEach((c: any) => c.value += characteristicMod);
				...
				copy.features
					.filter((f: any) => f.type === FeatureType.Ability)
					.map((f: any) => f.data.ability)
					.flatMap((a: any) => a.sections)
			...
			if (customization.convertToSolo) {
				...
				if (!copy.features.some((f: any) => f.name === 'End Effect')) {
					copy.features.push(FactoryLogic.feature.create({
